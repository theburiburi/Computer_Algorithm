...
int hash2(char str[], int random_value[])
{
    int i, h;

    for(i = 0, h = 0; i < strlen(str); i++)
    {
        h = (h + ((int)str[i] * random_value[i])) % TABLE_SIZE;
    }
    return h;
}
...
void hash_insert2(char name[], int random_value[])
{
    int number = 0;
    int key = hash2(name, random_value);
    while (strcmp(hash_table[key].name, "") != 0)
    {
        hash_table[key].collision++;
        number++;
...
int main()
{
    ...
    mt19937 gen(random());
    uniform_int_distribution <int> hk(0, TABLE_SIZE-1);

    int random_values[NAME_SIZE];
    for(int i=0; i < NAME_SIZE; i++)
        random_values[i] = hk(gen);
    ...
    while(getline(inputfile, line2))
    {
        ...
        for (i=0; line2[i] != '\0'; i++)
            get_name[i] = line2[i];
        get_name[i] = '\0';
        hash_insert2(get_name, random_values);
    }
